...
interface SidebarProps {
  ...
  datasetAId: string | null;
  datasetBId: string | null;
  onSelectDatasetA: (id: string) => void;
  onSelectDatasetB: (id: string | null) => void;
  mergeMode: boolean;
  onToggleMerge: () => void;
  filters: Filters;
            ...
            <div className="space-y-3">
                ...
                {datasetOptions.map((ds) => (
                  ...
                  >
                    <input
                      type="radio"
                      name="datasetA"
                      checked={datasetAId === ds.meta.id}
                      onChange={() => onSelectDatasetA(ds.meta.id)}
                      className="accent-primary"
                    />
                    <span className="truncate">{ds.meta.label}</span>
                  </label>
                ))}
              </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-200 dark:border-gray-700 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {datasetBId && (
                    <button onClick={() => onSelectDatasetB(null)} className="text-[11px] text-primary hover:underline">
                      limpar
                    </button>
                  )}
                </div>
                {datasetOptions.map((ds) => (
                  ...
                  >
                    <input
                      type="radio"
                      name="datasetB"
                      checked={datasetBId === ds.meta.id}
                      onChange={() => onSelectDatasetB(ds.meta.id)}
                      className="accent-primary"
                    />
                    <span className="truncate">{ds.meta.label}</span>
                  </label>
        ...
        <section>
          ...
          {isFilterOpen && (
            <div className="space-y-4">
              {[
                { key: 'accounts' as const, label: 'Conta de Anúncio' },
